...
def lint_arguments(
    ...
    args_list += cli_arguments.workflow_arguments("lint")
    args_list += ewoks_load_arguments()
    return args_list


def parse_lint_arguments(cli_args: Namespace, shell: bool = False) -> None:
    if shell:
    ...
    cli_args.lint_options = {"load_options": load_options}
