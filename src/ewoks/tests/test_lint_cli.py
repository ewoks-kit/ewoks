...
def test_lint_incorrect_graph(capsys):
    ...
    assert (
        "Input tag 'klass' found using 'task_type' does not match any of the "
        "expected tags: 'class', 'method', 'graph', 'ppfmethod', 'script', "
        "'notebook', 'generated', 'ppfport'" in captured.out
    )
